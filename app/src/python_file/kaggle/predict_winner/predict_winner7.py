...
from python_file.kaggle.predict_winner.kfold_target_encoding import (
...
def run_all():
    ...
    train_data = data[data["usage"] == 0].drop(["usage"], axis=1).reset_index(drop=True)
    test_data = data[data["usage"] == 1].drop(["usage"], axis=1).reset_index(drop=True)
    ids = test_data.loc[:, "id"]

    print(train_data, test_data)

    # KFoldTargetEncodingを実施
    categorical_columns = [x for x in data.columns if data[x].dtype == "object"]
    test_encoded_series = []
    encoded_cols = []
    for cat_col in categorical_columns:
        targetc = KFoldTargetEncoderTrain(cat_col, "y", n_fold=5)
        encoded_train = targetc.fit_transform(train_data)
        test_targetc = KFoldTargetEncoderTest(
            encoded_train, cat_col, f"{cat_col}_target_enc"
        )
        encoded_test = test_targetc.fit_transform(test_data)
        test_encoded_series.append(encoded_test.loc[:, f"{cat_col}_target_enc"])
        encoded_cols.append(f"{cat_col}_target_enc")
    tests = pd.concat(test_encoded_series, axis=1)
    test_data = test_data.drop(encoded_cols, axis=1)
    encoded_test = pd.concat([test_data, tests], axis=1).drop(
        categorical_columns, axis=1
    )

    train_x = encoded_train.drop([*categorical_columns, "y"], axis=1)
    train_y = encoded_train.loc[:, "y"]
    test_x = encoded_test.drop(["y"], axis=1)
    ...
    print("######################################")
    submission.to_csv(f"{DATA_DIR}/submission26.csv", index=False)
