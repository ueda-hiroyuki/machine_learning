...
def main():
    ...
    train_data, val_data = train_preprocess(train_x, train_y)
    model = train(train_data, val_data)
    y_pred = pd.DataFrame(predict(model, non_kesson_test))

    pred_df = pd.concat([test_id, y_pred], axis=1).rename(columns = {0 : 'SalePrice'}).set_index("Id")
    print(pred_df)
