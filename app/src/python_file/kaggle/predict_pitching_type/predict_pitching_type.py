...
def get_model(
    ...
    importance = pd.DataFrame(
        model.feature_importance(), index=cols, columns=["importance"]
...
def objective(X, y, trial):
    ...
    gbm.fit(tr_x, tr_y, eval_set=[(val_x, val_y)], early_stopping_rounds=20, verbose=50)
...
def main():
    ...
    test_pitch["use"] = "test"
    test_pitch["球種"] = 0
    pitch_data = pd.concat([train_pitch, test_pitch], axis=0).drop(
        PITCH_REMOVAL_COLUMNS, axis=1
    )

    player_data = pd.concat([train_player, test_player], axis=0).drop(
    ...
    pitching_type_ratio = pd.concat(
        [pitching_type_2016, pitching_type_2017, pitching_type_2018], axis=0
    ...
    train = (
        encorded_data[encorded_data["use"] == "train"]
        .drop("use", axis=1)
        .reset_index(drop=True)
    )
    test = (
        encorded_data[encorded_data["use"] == "test"]
        .drop("use", axis=1)
        .reset_index(drop=True)
    )
    ...
    for i, (tr_idx, val_idx) in enumerate(tscv.split(train_x_resampled)):
        ...
        val_dataset = lgb.Dataset(
            val_x, val_y, reference=tr_dataset, free_raw_data=False
        )
        model, evals_result = get_model(
            tr_dataset, val_dataset, num_class, best_params, train_x_resampled.columns
        )
