...
from sklearn.model_selection import train_test_split, KFold, GroupKFold, StratifiedKFold, cross_val_predict, TimeSeriesSplit
...
def main():
    ...
    gkf = GroupKFold(n_splits=n_splits)
    for i, (tr_idx, val_idx) in enumerate(gkf.split(pseudo_train_x, pseudo_train_y, groups=pseudo_train_x["投手ID"])):
    ...
    submission_df.to_csv(f"{DATA_DIR}/my_submission43.csv", header=False)
    print("#################################")
    print(submission_df)
    print(best_params) 
    print("#################################")
